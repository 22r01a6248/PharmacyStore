    ...
    ## CUSTOMERS
        ...
        if choice == "View":
            ...
            with st.expander("View All Customer Data"):
                cust_clean_df = pd.DataFrame(cust_result, columns=["Name", "Password", "Email", "Area", "Number"])
...
def customer(username, password):
    if get_authenticate(username, password):
        ...
        if st.button(label="Buy now"):
            ...
            O_Qty = str(dolo650) + ',' + str(strepsils) + ',' + str(vicks)
